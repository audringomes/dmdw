...
<xsl:stylesheet 
	...
	<xsl:variable name="MasterTemplateVersion" select="'$Rev: 845 $'"/>
	<xsl:variable name="XSLTProcessor" select="'Saxon-HE 9.8.0.1N from Saxonica'"/>
	...
	<xsl:template match="/">
		...
		<!-- A meta doc that shows the output as a link -->
			...
			<html> 			
				...
				<body onload="HighlightHash()">
					...
					<div class="ChapterInfo float">
						<xsl:call-template name="toc_info"/>
					</div>
					<nav class="float">
						<ul>
							<li><a href="{$filename2}">List of Collections</a></li>
							<li><a href="{$filename3}">Global Properties</a></li>
							<li><a href="{$filename4}">List of Enumerators</a></li>
							<li><a href="{$filename5}">Topological Interactions</a></li>
							<xsl:if test="/datamodel/soft_joins">
								<li><a href="{$filename6}">Magik joins</a></li>
							</xsl:if>
							<li><a href="{$filename1}">Miscellaneous</a></li>
						</ul>
					</nav>
		...
		<xsl:result-document href="{$filename1}" format="html">
			...
			<html>
				<xsl:call-template name="HtmlHeader">
					<xsl:with-param name="path" select="'../'"/>
					<xsl:with-param name="title" select="concat(/datamodel/@external_name,' - Technical details')"/>
				</xsl:call-template>
				<body onload="HighlightHash()">
					<xsl:call-template name="Top">
						<xsl:with-param name="path" select="'../'"/>
		...
		<xsl:result-document href="{$filename2}" format="html">
			...
			<html>
				<xsl:call-template name="HtmlHeader">
					<xsl:with-param name="path" select="'../'"/>
					<xsl:with-param name="title" select="concat(/datamodel/@external_name,' - List of Collections')"/>
				</xsl:call-template>
				<body onload="HighlightHash()">
					<xsl:call-template name="Top">
						<xsl:with-param name="path" select="'../'"/>
		...
		<xsl:result-document href="{$filename3}" format="html">
			...
			<html>
				<xsl:call-template name="HtmlHeader">
					<xsl:with-param name="path" select="'../'"/>
					<xsl:with-param name="title" select="concat(/datamodel/@external_name,' - Global properties')"/>
				</xsl:call-template>
				<body onload="HighlightHash()">
					<xsl:call-template name="Top">
						<xsl:with-param name="path" select="'../'"/>
		...
		<xsl:result-document href="{$filename4}" format="html">
			...
			<html>
				...
				<xsl:call-template name="HtmlHeader">
					<xsl:with-param name="path" select="'../'"/>
					<xsl:with-param name="title" select="concat(/datamodel/@external_name,' - List of Enumerators')"/>
				</xsl:call-template>
		...
		<xsl:result-document href="{$filename5}" format="html">
		  ...
		  <html>
		    ...
		    <xsl:call-template name="HtmlHeader">
		      <xsl:with-param name="path" select="'../'"/>
		      <xsl:with-param name="title" select="concat(/datamodel/@external_name,' - Topological Interactions')"/>
		    </xsl:call-template>
			...
			<xsl:with-param name="path" select="'../'"/>
		      ...
		      <div class="PageTitle float">Topology Interactions</div>
		      <div class="ChapterInfo float">
			<xsl:call-template name="topology_info"/>
		      </div>

		      <div>
			<xsl:choose>
			  <xsl:when test="count(//topology_table) gt 100">
			    <xsl:call-template name="PageWithFilter">
			      <xsl:with-param name="elemclass" select="'dmTopology'"/>
			      <xsl:with-param name="selectList" select="//topology_table"/>
			      <xsl:with-param name="select" select="datamodel/topology_by_manifold"/>
			    </xsl:call-template>
			  </xsl:when>
			  <xsl:otherwise>
			    <xsl:call-template name="TablesHavingTopology"/>
		...
		<xsl:if test="/datamodel/soft_joins">
			<xsl:result-document href="{$filename6}" format="html">
				...
				<html>
					...
					<xsl:call-template name="HtmlHeader">
						<xsl:with-param name="path" select="'../'"/>
						<xsl:with-param name="title" select="concat(/datamodel/@external_name,' - Soft Joins')"/>
					</xsl:call-template>
		...
		<table class="dmTopology float">
			...
			<xsl:for-each select="topology_interaction">
				<tr>
					...
					<xsl:attribute name="id">
						<xsl:value-of select="parent::topology_table/attribute::name"/>
						<xsl:text>.</xsl:text>
	...
	<xsl:template match="soft_join">
		...
		<tr class="SoftJoin VisFalse">
			<td class="technical"><xsl:value-of select="join_name"/></td>
...
<!-- NAMED TEMPLATES -->
			...
			<xsl:variable name="filename" select="concat($path_prefix,@name,'.html')"/>
				...
				<xsl:result-document href="{$filename}" format="html">
				  ...
				  <html>
					...
					<xsl:call-template name="HtmlHeader">
						<xsl:with-param name="path" select="'../'"/>
						<xsl:with-param name="title" select="concat(/datamodel/@external_name,' - Collection ',@external_name)"/>
					</xsl:call-template>
	...
	<xsl:template name="TablesHavingTopology">
	  <!-- Creates a list of collections that has geometries with topology -->
	  <div style="overflow-x:auto">
	    <table class="dmtable float">
	      <tr>
		<th colspan="2"><xsl:text>Tables with toplogical interaction</xsl:text></th>
	      </tr>
	      <tr>
		<th>Manifold</th>
		<th>Table</th>
	      </tr>
	      <xsl:for-each select="/datamodel/topology_by_manifold/topology/topology_table">
		<tr>
		  <td>
		    <xsl:value-of select="../@manifold_name"/>
		  </td>
		  <td>
		    <a href="#{@name}" onclick="HighLightInPage('{@name}')">
		      <xsl:value-of select="@table_external_name"/>
		    </a>
		  </td>
		</tr>
